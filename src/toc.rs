...
/// Removes empty anchor links like `[](#anchor-name)` from heading text.
///
/// These are commonly generated by static site generators for heading permalinks
/// but add no value in a table of contents since the `ToC` itself provides navigation.
fn strip_empty_anchors(s: &str) -> String {
...
///
/// Uses pulldown-cmark to identify headings, then retrieves the original
/// line text with empty anchor links removed.
///
/// Algorithm:
...
/// 4. Return trimmed line text with `[](#anchor)` patterns stripped
///
/// Preserves formatting (bold, italic, links with text) but removes redundant empty anchors.
    ...
    for (event, range) in parser {
        if let Event::Start(Tag::Heading { level, .. }) = event {
            ...
            if let Some((_, line_text)) = lines_with_offsets.get(line_idx) {
                let cleaned = strip_empty_anchors(line_text);
                if !cleaned.is_empty() {
...
mod tests {
    ...
    #[test]
    fn test_strip_empty_anchors() {
        assert_eq!(strip_empty_anchors("# Heading"), "# Heading");
        assert_eq!(
            strip_empty_anchors("## Writing markup with JSX [](#writing-markup-with-jsx)"),
            "## Writing markup with JSX"
        );
        assert_eq!(
            strip_empty_anchors("### Multiple []( #anchor1) and [](#anchor2)"),
            "### Multiple  and"
        );
        assert_eq!(strip_empty_anchors("[](#only-anchor)"), "");
        // Preserve non-empty links
        assert_eq!(
            strip_empty_anchors("# Link [text](url) preserved"),
